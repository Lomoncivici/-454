        ...
        with self._connection:
            ...
            self._connection.execute("""
                CREATE TABLE IF NOT EXISTS element(
                    element_id INTEGER PRIMARY KEY AUTOFICATION,
                    cloth_name TEXT,
                    size_cloth INTEGER,              
                    quantity INTEGER,
                    price REAL
           ...
           """ )
            ...
            self._connection.execute("""
                CREATE TABLE IF NOT EXISTS card(
                    card_id INTEGER PRIMARY KEY AUTOFICATION,
                    user_login TEXT,
                    element_id INTEGER,
                    size_cloth INTEGER,  
                    quantity INTEGER,
        ...
        with self._connection:
            self._connection.execute("""
                CREATE TABLE IF NOT EXISTS delevery(
                    delevery_id INTEGER PRIMARY KEY AUTOFICATION,
                    user_login TEXT,
                    address TEXT,
                    number_zakaza INTEGER,
                    FOREING KEY (user_login) FREFERENCES user (Login),
                    FOREING KEY (number_z) FREFERENCES zakaz (number_z),
                )
            """) 

    def _populate_sample_element(self):
        cloth = [
            ('Пальто', 30-47, 45, 7000),
            ('кофта', 60-89, 58, 2799),
            ('штаны', 35-56, 48, 899),
            ('форма', 45-98, 4654, 645-4681),
            ('майки', 13-23, 610, 187-4681),
            ('шорты', 20-96, 4152, 915-5942),
        ]     
